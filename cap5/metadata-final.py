...
def read_metadata(filename):
    ...
    for coluna in read_lines(filename):
        if coluna:
            metadata.append(tuple(coluna.split('\t')[:3]))
    return metadata
...
def prompt():
    ...
    print("(d) Exibir atributos de entidade")
    print("(r) Exibir referências de entidade")
...
def main():
        ...
        for col in val:
            if col[0] in keys:
                if not col[0] == meta[key][0][0]:
                    relacionamentos[key] = keys[col[0]]
    ...
    while opcao != 's':
        if opcao == 'l':
            ...
            entity_name = input("Nome da entidade: ")
            other_entity = relacionamentos[entity_name]
            print(other_entity)
        else:
            print("Inexistente\n ")
        opcao = prompt()
